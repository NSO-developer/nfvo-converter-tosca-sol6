...
class TestVPCDI(unittest.TestCase):
    ...
    def test_vdu_length(self):
        self.assertEqual(len(self.vdus), 4)
    ...
    def test_vdu_storage_desc(self):
        ...
        for i, vdu in enumerate(self.vdus):
            with self.subTest(i=i):
                if vdu["id"] in positive:
                    self.assertIn("virtual-storage-desc", vdu)
                else:
                    self.assertNotIn("virtual-storage-desc", vdu)
    ...
    def test_vdu_compute_desc(self):
        ...
        for i, vdu in enumerate(self.vdus):
            with self.subTest(i=i):
                if vdu["id"] in positive:
                    self.assertIn("virtual-compute-desc", vdu)
                else:
                    self.assertNotIn("virtual-compute-desc", vdu)
